    ...
    <section id="skills" className="py-28" style={{ backgroundColor: 'var(--bg-surface)' }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 grid sm:grid-cols-2 gap-6 items-stretch auto-rows-fr">
            {skillGroups.map((group, gi) => (
              <Reveal key={group.title} delay={gi * 100} className="h-full">
                <div
                  className="h-full flex flex-col p-6 rounded-2xl border hover-lift"
                ...
                >
                  ...
                  <div className="flex flex-wrap content-start gap-2">
                    {group.items.map((item, i) => (
                      <span
                        key={item}
                        className="text-xs font-mono px-3 py-1.5 rounded-full border tag-pop transition-transform hover:scale-110"
                        style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)', animationDelay: `${gi * 100 + i * 40}ms` }}
                      >
                        {item}
                      </span>
                    ))}
          ...
          <div className="space-y-6">
            <Reveal delay={150} direction="right">
              <div className="p-6 rounded-2xl border" style={{ backgroundColor: 'var(--accent-soft)', borderColor: 'var(--accent)' }}>
                <div className="flex items-center gap-2 mb-4" style={{ color: 'var(--accent-text)' }}>
                  <Users size={18} />
                  <h3 className="font-display font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>Competências pessoais</h3>
                </div>
                <ul className="space-y-3">
                  {softSkills.map((skill) => (
                    <li key={skill} className="flex items-start gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
